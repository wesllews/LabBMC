	...
	with  open("../SQL/10.genotypes.sql", 'w') as file:
		for row in read_genotipos:
			...
			for i in range (1,len(locus)): 
				...
				if alelo !="0":
					sql = f"INSERT INTO `genotype` (`id_individual`, `id_locus`, `allele`, `restrict`) VALUES ((SELECT id FROM individual WHERE identification='{id_individual}'), (SELECT id FROM locus WHERE locus='{id_locus}'), '{alelo}', 'n') ;\n"		
					file.write(sql)
