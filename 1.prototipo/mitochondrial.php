...
if(in_array("delete",$_SESSION['permission'])){
	...
	$sql_locus = "SELECT DISTINCT(mitochondrial_locus) FROM haplotype INNER JOIN mitochondrial_locus ON mitochondrial_locus.id = haplotype.id_mitochondrial_locus";
	$query = $mysqli->query($sql_locus);
	while ($row = $query->fetch_array()) {
	    array_push($headersAdicionais, $row["mitochondrial_locus"]); //Sunstituindo . por _ para envio de variáveis pro método get
	}
...
$sql= "SELECT * FROM(
SELECT DISTINCT(haplotype.id_individual) as id, identification, id_category, category, sex,
CASE
...
INNER JOIN individual ON individual.id=haplotype.id_individual
INNER JOIN status ON status.id_individual=haplotype.id_individual
INNER JOIN category ON individual.id_category=category.id
LEFT JOIN institute ON status.id_institute=institute.id
LEFT JOIN fragment ON status.id_fragment=fragment.id)haplotype2 WHERE 1=1";
...
<div class="text-warning m-3" style="white-space: nowrap;"><h3 class="ml-5">Mitochondrial</h3><hr></div>
		...
		<div class="modal-dialog modal-dialog modal-lg" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="formFiltro" action="mitochondrial.php" method="get">
						...
						<div class="form-group">
							...
							<select name="filterpopulation" class="form-control form-control-sm">
							  ...
							  SELECT DISTINCT(haplotype.id_individual) as id, identification, id_category, sex, alive,
							  CASE
							      WHEN id_category = 1 THEN institute.abbreviation
							      WHEN id_category = 2 THEN fragment.fragment
							  END AS population
							  ...
							  INNER JOIN status ON status.id_individual=haplotype.id_individual
							  LEFT JOIN institute ON status.id_institute=institute.id
							  LEFT JOIN fragment ON status.id_fragment=fragment.id)haplotype2";
				...
				<div class="modal-footer">
					...
					<form id="formClear" action="mitochondrial.php" method="get">
			...
			<?php foreach ($header as $value): ?>
				<th scope="col" style="white-space: nowrap;">
					<div class="d-flex justify-content-center align-items-end">
						...
						<?php else: ?>
							<?php
							$sql="SELECT * FROM mitochondrial_locus WHERE mitochondrial_locus='$value'";
			...
			<tr class="text-center">
				<?php foreach ($header as $value):?>
					<?php switch ($value):
									...
									<div class="modal fade" id="population<?php echo str_replace(' ','_',$row['population']); ?>" tabindex="-1" role="dialog" aria-hidden="true">
										<div class="modal-dialog modal-dialog-centered" role="document">
											<div class="modal-content">
												...
												<div class="modal-footer">
													<form action="mitochondrial.php" method="get">
	    				...
	    				<?php case 'manager': ?>
	    					<td scope="row">
			    				<form action="haplotype_edit.php" method="GET" id="edit<?php echo $row['id'];?>" target="_blank">
						...
						<?php default:
						 	$sql_locus = "SELECT *, haplotype.id as id FROM haplotype WHERE restricted ='0' AND id_individual='$row[id]' AND id_mitochondrial_locus=(SELECT id FROM mitochondrial_locus WHERE mitochondrial_locus='$value'); ";
						 	$result_locus = $mysqli->query($sql_locus);
							if ($result_locus->num_rows>0):?>
								<td>
									<?php $row_locus = $result_locus->fetch_array();?>
									<textarea class="form-control bg-light border-0 fasta" id="haplotype<?php echo $row_locus['id']; ?>" onclick="copy('haplotype<?php echo $row_locus['id'];?>');" data-toggle="tooltip"  title="Copied" data-trigger="focus" readonly><?php echo $row_locus['haplotype']; ?></textarea>
								</td>
							<?php else: ?>
								<td scope="row">-</td>
							<?php endif;?>
...
<!-- Download -->
	<form id="formDownload" action="download_mitochondrial.php" method="post">
		<input type="hidden" name="pagina" value="haplotype">	
