...
/* Cabeçalho da tabela */
$header = ['name','abbreviation','country','state','city','alive_individuals'];
...
$sql= "SELECT * FROM `institute` WHERE 1=1";
$sql_pagination = $sql;
$sql_filter = $sql_pagination.$order.$limit_sql;
$result_filter = $mysqli->query($sql_filter);
?>

<div class="text-warning m-3" style="white-space: nowrap;"><h3 class="ml-5">Institutes</h3><hr></div>
			...
			<tbody>
				...
				<tr class="text-center">
					...
					<td scope="col">
						...
						<?php switch ($value):
							case 'name': ?>
			    				<form action="captivity.php" method="GET" id="view<?php echo $row['id'];?>" target="_blank">
			    					<input type="hidden" name="filterpopulation" value='<?php echo $row['id'];?>'>
			    					<input type="hidden" name="fulldata" value='n'>
			    					<button type="submit" form="view<?php echo $row['id'];?>" class="btn btn-sm btn-outline-success btn-block border-0"><?php echo $row[$value];?></button>
			    				</form>
		    				<?php break;?>
							
							<?php case 'alive_individuals':
								$query = "SELECT count(*) as num FROM `status` where alive=1 and id_institute='$row[id]'";
								$result = $mysqli->query($query);
								$row_alive = $result->fetch_array();?>
								<?php if($row_alive["num"]>0):?>
									<form action="captivity.php" method="GET" id="alive<?php echo $row['id'];?>" target="_blank">
				    					<input type="hidden" name="filterpopulation" value='<?php echo $row['id'];?>'>
				    					<input type="hidden" name="fulldata" value='n'>
				    					<input type="hidden" name="status" value='1'>
				    					<button type="submit" form="alive<?php echo $row['id'];?>" class="btn btn-outline-primary btn-block border-0"><?php echo $row_alive["num"];?> Individuals </button>
				    				</form>
				    			<?php else: ?>
				    				<?php echo $row_alive["num"];?> Individuals
				    			<?php endif; ?>
							<?php break; ?>
							...
							<?php case 'manager': ?>
			    				<form action="delete.php" method="GET" id="delete<?php echo $row['id'];?>" target="_blank">
			    					...
			    					<input type="hidden" name="delete" value='institute'>
			    				</form>
			    				<form action="institute_insert.php" method="GET" id="edit<?php echo $row['id'];?>" target="_blank">
			    					<input type="hidden" name="id" value="<?php echo $row['id'];?>">
